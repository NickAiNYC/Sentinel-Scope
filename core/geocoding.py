...
class SentinelBatchProcessor:
    ...
    Optimized for NYC Building Code 2022/2025 compliance reasoning.
    """
    ...
    def __init__(self, engine: ComplianceGapEngine, api_key: str, max_workers: int = 5):
        ...
        # 2025 Standard: Strict instructions to prevent model hallucination
        self.system_prompt = """
        ACT AS: A Senior NYC DOB Forensic Inspector.
        TASK: Conduct a visual milestone audit using NYC BC 2022/2025 guidelines.
        
        REQUIRED FIELDS IN JSON:
        - milestone: (string) Specific BC reference
        - floor: (string) Verified floor code
        - confidence: (float 0.0-1.0)
        - evidence_notes: (string) Narrative for DOB audit trail
    ...
    def _process_single_image(self, file_source: Union[str, Any]) -> CaptureClassification:
        ...
        try:
            ...
            # DeepSeek-V3.2 uses OpenAI-compatible Vision payload
            response = self.client.chat.completions.create(
                model="deepseek-chat", # Ensure this supports vision in your tier
            ...
            # Forensic Validation (Pydantic v2)
