...
try:
    from core.constants import BOROUGHS, DOB_CLASS_MAP, VALIDATION_PATTERNS
except ImportError:
...
class CaptureClassification(BaseModel):
    ...
    capture_timestamp: datetime | None = Field(
        None, description="When the photo was taken (if available from EXIF)"
    )

    image_hash: str | None = Field(
        None, description="SHA-256 hash of source image for deduplication"
    )
    ...
    def normalize_milestone(cls, v: str) -> str:
        ...
        replacements = {"Mep": "MEP", "Hvac": "HVAC", "Dob": "DOB", "Nyc": "NYC"}
...
class ComplianceGap(BaseModel):
    ...
    milestone: str = Field(
        ..., description="Missing milestone name", min_length=2, max_length=100
    )
    ...
    risk_level: str = Field(
        ..., pattern=r"^(Critical|High|Medium|Low)$", description="Risk severity level"
    )
    ...
    estimated_cost: float | None = Field(
        None, ge=0, description="Estimated remediation cost (USD)"
    )
...
class GapAnalysisResponse(BaseModel):
    ...
    missing_milestones: list[ComplianceGap] = Field(
        default_factory=list, description="List of detected compliance gaps"
    )

    compliance_score: int = Field(
        ..., ge=0, le=100, description="Overall compliance score (0-100%)"
    )

    risk_score: int = Field(
        ..., ge=0, le=100, description="Overall risk score (0-100, higher = more risk)"
    )

    total_found: int = Field(..., ge=0, description="Total milestones detected")

    gap_count: int = Field(..., ge=0, description="Total gaps identified")

    next_priority: str = Field(..., description="Next recommended action or priority")
    ...
    project_name: str | None = Field(None, description="Project identifier")
    ...
    def check_score_logic(self) -> Self:
        ...
        """
        has_critical = any(
            gap.risk_level == "Critical" for gap in self.missing_milestones
        )
        ...
        if has_critical:
            ...
            try:
                critical_gap = next(
                    g for g in self.missing_milestones if g.risk_level == "Critical"
                )
                self.next_priority = (
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        critical_count = sum(
            1 for g in self.missing_milestones if g.risk_level == "Critical"
        )
        high_count = sum(1 for g in self.missing_milestones if g.risk_level == "High")
