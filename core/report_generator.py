...
class SentinelReportGenerator:
    ...
    def generate_report(
        ...
        milestone_classifications = [
            r.get("milestone_classification") for r in batch_results
            if r.get("milestone_classification")
        ]
        esg_analyses = [
            r.get("sustainability_analysis") for r in batch_results
            if r.get("sustainability_analysis")
        ]

        high_esg_risks = sum(1 for e in esg_analyses if e and e.get("energy_risk_level") == "High")
        ...
        if gap_analysis:
            ...
            self._add_executive_summary(pdf, compliance_score, risk_score, gap_count, next_priority,
                                       len(batch_results), len(milestone_classifications), 
                                       high_esg_risks, include_sustainability)
        else:
            self._add_fallback_summary(pdf, len(batch_results), len(milestone_classifications))
    ...
    def _add_executive_summary(self, pdf: FPDF, compliance_score: int, risk_score: int, 
                              gap_count: int, next_priority: str, total_images: int, 
                              classified_images: int, high_esg_risks: int, include_sustainability: bool):
        ...
        self._add_metric_box(pdf, "Compliance Score", f"{compliance_score}%", 
                           (30, 64, 175), x=10)
        
        # Risk Score box
        risk_color = (5, 150, 105) if risk_score < 30 else (220, 38, 38)
        self._add_metric_box(pdf, "Risk Score", f"{risk_score}/100", 
                           risk_color, x=60)
        
        # Gaps Detected box
        self._add_metric_box(pdf, "Gaps", str(gap_count), 
                           (245, 158, 66), x=110)
        
        # Images Analyzed box
        self._add_metric_box(pdf, "Images", str(total_images), 
                           (124, 58, 237), x=160)
        ...
        if include_sustainability:
            if high_esg_risks > 0:
                ...
                pdf.cell(0, 8, f"⚠️ {high_esg_risks} image(s) with HIGH Local Law 97 risk", 0, 1)
            else:
    ...
    def _add_fallback_summary(self, pdf: FPDF, total_images: int, classified_images: int):
    ...
    def _add_metric_box(self, pdf: FPDF, label: str, value: str, color: tuple, x: float):
    ...
    def _add_compliance_section(self, pdf: FPDF, gap_analysis: Any):
        ...
        pdf.cell(0, 8, f"Found: {total_found} milestones • Missing: {len(missing_milestones)} milestones", 0, 1)
        ...
        if missing_milestones:
            ...
            pdf.cell(0, 10, "✅ No compliance gaps detected. Excellent documentation!", 0, 1)
            pdf.set_text_color(0, 0, 0)
    ...
    def _add_esg_section(self, pdf: FPDF, esg_analyses: List[Dict]):
        ...
        pdf.set_font('Helvetica', '', 12)
        pdf.set_text_color(0, 0, 0)
        pdf.cell(0, 8, f"Visual energy efficiency assessment performed on {len(esg_analyses)} images.", 0, 1)
        
        # Count risk levels
        risk_counts = Counter(a.get("energy_risk_level", "Unknown") for a in esg_analyses if a)
        ...
        pdf.cell(0, 8, "Recommendation: Consider professional energy audit for full LL97 decarbonization planning.", 0, 1)
        pdf.set_text_color(0, 0, 0)
        pdf.ln(10)

    def _add_photo_summary(self, pdf: FPDF, total_images: int, milestone_classifications: List):
        ...
        pdf.set_font('Helvetica', '', 12)
        pdf.set_text_color(0, 0, 0)
        pdf.cell(0, 8, f"{total_images} site photos processed using DeepSeek-VL multimodal AI.", 0, 1)
    ...
    def _add_footer(self, pdf: FPDF):
        ...
        pdf.cell(0, 5, f"Auditor: {self.auditor_name} • Powered by DeepSeek AI Vision • © 2026 ThriveAI", 0, 1, 'C')
